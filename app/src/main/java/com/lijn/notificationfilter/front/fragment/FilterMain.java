...
// 设置监听器
    ...
    private void setSwitchListeners() {
        ...
        logUnknownPassedNotificationSwitch.setOnCheckedChangeListener((buttonView, isChecked) -> {
            ProgramSettingManager.getInstance().getProgramSetting().setLogNotificationVariety(NotificationType.UNCHECKED);
            // 更新设置
        });

        filterRulesSwitch.setOnCheckedChangeListener((buttonView, isChecked) -> {
            if (isChecked) {
                ProgramSettingManager.getInstance().getProgramSetting().setFilterVariety(FilterType.RULE, true);
            } else {
                ProgramSettingManager.getInstance().getProgramSetting().setFilterVariety(FilterType.RULE, false);
            }
        });
